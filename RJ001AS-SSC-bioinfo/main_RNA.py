...
from common.base_pipeline import BasePipeline, run_via_snakemake, run_via_shellscript
...
DEFAULT_CONFIG_FILE = f'{PIPELINE_ROOT}/config/thyoid_A2.yaml'
...
class RNAPipeline(BasePipeline):
    pipeline_name = "rna_pipeline"
    main_workflow = "workflows/RNA/all.smk"
    module_dict = OrderedDict([
        ("trim", "workflows/RNA/trim.smk"),
        ("bwa", "workflows/RNA/bwa.smk"),
        ("filter", "workflows/RNA/bam_filter.smk"),
        ("anno", "workflows/RNA/anno.smk"),
        ("stat", "workflows/RNA/qc_stat.smk"),
        ("clear", "workflows/RNA/clear.smk")
    ])
...
def main():
    ...
    if not args.via_snakemake:
        run_via_shellscript(args.sample_name, args.read1, args.read2, out_dir, config_file, run_mode, CONFIG_DICT["snakemake"], PIPELINE_ROOT, RNAPipeline)
    else:
        run_via_snakemake(args.sample_list, out_dir, config_file, run_mode, CONFIG_DICT['snakemake'], PIPELINE_ROOT, RNAPipeline)
